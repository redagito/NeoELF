ELF_API elfLight* elfCreateLight(const char* name)
{
    elfLight* light;
...
ELF_API void elfSetLightType(elfLight* light, int type)
{
    if (type != ELF_POINT_LIGHT && type != ELF_SPOT_LIGHT && type != ELF_SUN_LIGHT)
        return;
    light->lightType = type;
}
ELF_API void elfSetLightColor(elfLight* light, float r, float g, float b, float a)
{
    light->color.r = r;
    light->color.g = g;
    light->color.b = b;
...
ELF_API void elfSetLightRange(elfLight* light, float range, float fadeRange)
{
    light->range = range;
...
ELF_API void elfSetLightShadows(elfLight* light, unsigned char shadows)
{
...
ELF_API void elfSetLightVisible(elfLight* light, unsigned char visible) { light->visible = !(visible == ELF_FALSE); }
ELF_API void elfSetLightCone(elfLight* light, float innerCone, float outerCone)
{
...
ELF_API void elfSetLightShaft(elfLight* light, unsigned char shaft) { light->shaft = !shaft == ELF_FALSE; }
ELF_API void elfSetLightShaftSize(elfLight* light, float size)
{
    light->shaftSize = size;
    if (light->shaftSize < 0.0f)
        light->shaftSize = 0.0f;
}
ELF_API void elfSetLightShaftIntensity(elfLight* light, float intensity)
{
    light->shaftIntensity = intensity;
    if (light->shaftIntensity < 0.0f)
        light->shaftIntensity = 0.0f;
}
ELF_API void elfSetLightShaftFadeOff(elfLight* light, float fadeOff)
{
    light->shaftFadeOff = fadeOff;
    if (light->shaftFadeOff < 0.0f)
        light->shaftFadeOff = 0.0f;
}
ELF_API int elfGetLightType(elfLight* light) { return light->lightType; }
ELF_API elfColor elfGetLightColor(elfLight* light) { return light->color; }
ELF_API float elfGetLightRange(elfLight* light)
{
    return light->range;
    ELF_API float elfGetLightFadeRange(elfLight * light) { return light->fadeRange; }
    ELF_API unsigned char elfGetLightShadows(elfLight * light) { return light->shadows; }
    ELF_API unsigned char elfGetLightVisible(elfLight * light) { return light->visible; }
    ELF_API float elfGetLightInnerCone(elfLight * light) { return light->innerCone; }
    ELF_API float elfGetLightOuterCone(elfLight * light) { return light->outerCone; }
    ELF_API unsigned char elfGetLightShaft(elfLight * light) { return light->shaft; }
    ELF_API float elfGetLightShaftSize(elfLight * light) { return light->shaftSize; }
    ELF_API float elfGetLightShaftIntensity(elfLight * light) { return light->shaftIntensity; }
    ELF_API float elfGetLightShaftFadeOff(elfLight * light) { return light->shaftFadeOff; }
